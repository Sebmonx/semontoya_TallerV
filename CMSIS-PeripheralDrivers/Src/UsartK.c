...
void USART_Config(USART_Handler_t *ptrUsartHandler){
	...
	/* Por ultimo activamos el modulo USART cuando esta correctamente configurado */
	...
	if(ptrUsartHandler->USART_Config.USART_datasize == USART_DATASIZE_8BIT){
		ptrUsartHandler->ptrUSARTx->CR1 &= ~(USART_CR1_M);   //8 data bits = 0

	}else{
		ptrUsartHandler->ptrUSARTx->CR1 |= (USART_CR1_M);    //9 data bits = 1
	}
	...
	/*************NOTA IMPORTANTE*************/
	/*Si el pll está apagado, se entiende que el HSI es el que tiene el microcontrolador por defecto
	 *
	 *Si el pll está encendido, tendrá por defecto la frecuencia conigurada (En este caso 80MHZ)    */
//	if(RCC->CFGR &= ~(RCC_CR_PLLON)){
//			// 2.5.1 Configuracion del Baudrate (SFR USART_BRR)
//			// Ver tabla de valores (Tabla 73), Frec = 16MHz, overr = 0;
	if(ptrUsartHandler->USART_Config.USART_baudrate == USART_BAUDRATE_9600){
		...
		ptrUsartHandler->ptrUSARTx->BRR = 0x0683;
			/*Si se usa el USART2 con el pll activo, los calculos se hacen a 40MHz**/
//		ptrUsartHandler->ptrUSARTx->BRR = 0x1047;
		...
		ptrUsartHandler->ptrUSARTx->BRR = 0x8B;
			// Si es para 40 MHz en usart 2
//		ptrUsartHandler->ptrUSARTx->BRR = 0x15B;
	}

//	if(RCC->CFGR |= (RCC_CR_PLLON)){
		 //2.5.2 Configuracion del Baudrate (SFR USART_BRR)
		 //Ver tabla de valores (Tabla 73), Frec = 80MHz, overr = 0;
//	if(ptrUsartHandler->USART_Config.USART_baudrate == USART_BAUDRATE_9600){
...
/* Write mesage DUDA!!.También se hace en ese registro? */
